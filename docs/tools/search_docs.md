...
`search_docs` performs a targeted search within identified documentation files (READMEs and `docs/*.md`). Unlike standard `grep`, it ranks results based on a **relevance score** and provides optional **surrounding context lines** to help you understand the match without reading the full file.
