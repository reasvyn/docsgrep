...
`spot_delta` provides a detailed diff between what a documentation file claims and what the code actually implements. It's similar to `verify_truth` but focuses on highlighting the specific implementation details (including code snippets) that have diverged from the documentation.
