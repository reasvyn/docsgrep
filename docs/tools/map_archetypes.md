...
`map_archetypes` is an architectural advisor tool that analyzes the "backbone" of your project. It identifies repeated structural patterns across multiple files and suggests when to extract logic into a **Base Class**, **Trait**, or **Interface**.
...
```bash
# Run via CLI
docsgrep run map_archetypes --dirPath ./src --minSimilarity 0.7
```
