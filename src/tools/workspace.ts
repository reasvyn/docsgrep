...
export async function handleSetupCamp(args: any): Promise<any> {
  ...
  try {
    ...
    return {
      content: [{ type: "text", text: `Error initializing workspace: ${error.message}` }],
      isError: true,
    };
  }
}

export async function handlePurgeCache(args: any): Promise<any> {
  ...
  try {
    ...
    return {
      content: [{ type: "text", text: `Error cleaning cache: ${error.message}` }],
      isError: true,
    };
  }
}
