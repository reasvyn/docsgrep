...
/**
 * Resolve the cache path for a cloned repository.
 *
 * Always uses the project-local `.docsgrep/repos/` directory.
 * The `localProjectPath` parameter is the project root containing `.docsgrep/`.
 * If not provided, falls back to cwd.
 */
export function getRepoCachePath(
  ...
  const projectRoot = localProjectPath || process.cwd();
  const baseReposDir = path.join(projectRoot, ".docsgrep", "repos");
  ...
  return path.join(baseReposDir, `${repoName}${versionSuffix}-${repoHash}`);
}
